<?php defined('G_IN_SYSTEM')or exit('No permission resources.'); ?><div class="footer_content">
   		...
   		<div class="support">
            ...
            <?php if(defined('G_IN_ADMIN')) {echo '<div style="padding:8px;background-color:#F93; color:#fff;border:1px solid #f60;text-align:center"><b>This Tag</b></div>';}?>
   			...
   			<dl class="ft-weixin">
   				...
   				<dd class="gray02">关注{wc:_cfg("web_name_two")}免费抽大奖</dd>
   			</dl>
   		</div>
   	</div>
   	<div class="service-promise">
   		<ul>
   			...
   			<li class="sp-business service-promise-border-r0"><a href="<?php echo WEB_PATH; ?>/single/business" target="_blank"><span>商务合作023-67898642</span></a></li>
   		</ul>
   	</div>
...
var time_arr = new Array();
	time_arr['obj'] = $("#sp_ServerTime");
	time_arr['time'] = <?php echo time(); ?>*1000;	
	time_arr['H'] = "";
	time_arr['I'] = "";
	time_arr['S'] = "";	
...
function showTime(){		
		...
		*/
		time_arr['tobj'] = new Date(time_arr['time']);
		time_arr['H'] = (time_arr['tobj'].getHours()<10)  ? ('0'+time_arr['tobj'].getHours()) : (time_arr['tobj'].getHours());
		time_arr['I'] = (time_arr['tobj'].getMinutes()<10)?('0'+time_arr['tobj'].getMinutes()):(time_arr['tobj'].getMinutes());
		time_arr['S'] = (time_arr['tobj'].getSeconds()<10)?('0'+time_arr['tobj'].getSeconds()):(time_arr['tobj'].getSeconds());	
	
		time_arr['obj'].text(time_arr['H']+':'+time_arr['I']+':'+time_arr['S']);
		time_arr['time'] += 1000;
}
