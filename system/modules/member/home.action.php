...
class home extends base {
	...
	public function userphotoinsert(){				
		...
		if(isset($_POST["submit"])){		
			$_POST['img'] = $tname  = trim(str_ireplace(" ","",$_POST['img']));	
	...
	public function mailchackajax(){
		$mysql_model=System::load_sys_class('model');
		$member=$this->userinfo;
		$member2=$mysql_model->GetOne("select * from `@#_member` where `email`='".$_POST['param']."'");
		if(!empty($member2)){
			echo "邮箱已经存在";			
		}else{
			echo '{
	...
	public function sendsuccess(){		
		if(!isset($_POST['submit']))_message("参数错误");
		if(!isset($_POST['email']) || empty($_POST['email']))_message("邮箱地址不能为空!");
		if(!_checkemail($_POST['email']))_message("邮箱格式错误!");
	...
	public function mobilesuccess(){
		...
		if(isset($_POST['submit'])){
			...
			$member2=$this->db->GetOne("select * from `@#_member` where `mobile`='$mobile' and `uid` != '$member[uid]'");
			if($member2 && $member2['mobilecode'] == 1){
				_message("手机号已被注册！");
			}
			if($mobile==null){
				_message("手机号不能为空",null,3);
			}
...
//福分、经验添加			
	...
	public function singlelist(){
		...
		$record=$this->db->Getlist("select * from `@#_member_go_record` where shopid in ($rd_id) and `uid`='$member[uid]' and `huode`>'10000000'");
