...
class member extends admin {
	...
	public function __construct(){
		...
		$this->ment=array(
						...
						array("insert","会员福利配置",ROUTE_M.'/'.ROUTE_C."/member_fufen"),
	...
	public function lists(){
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `$table` WHERE 1"); 
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");		
		$members=$this->db->GetPage("SELECT * FROM `$table` WHERE 1",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
	...
	public function insert(){
		...
		if(isset($_POST['submit'])){
			...
			$time = time();
			$sql="INSERT INTO `@#_member` (username,img,email,mobile,password,money,jingyan,emailcode,mobilecode,qianming,groupid,time) value ('$username','$img','$email','$mobile','$password','$money','$jingyan','$emailcode','$mobilecode','$qianming','$membergroup','$time')";
			$this->db->Query($sql);
	...
	public function modify(){
		...
		if(isset($_POST['submit'])){
			...
			$qianming=htmlspecialchars(trim($_POST['qianming']));
			$membergroup=htmlspecialchars(trim($_POST['membergroup']));			
			$sql="UPDATE `@#_member` SET `username`='$username',`email`='$email',`mobile`='$mobile',`password`='$password',`money`='$money',`emailcode`='$emailcode',`mobilecode`='$mobilecode',`img`='$img',`groupid`='$membergroup',`qianming`='$qianming' WHERE `uid`='$uid'";
	...
	public function select(){
		if(isset($_POST['submit'])){
			...
			if($sousuo=='id'){			
				$arr=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` = '$content'");				
			}
			if($sousuo=='nickname'){	
				$arr=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `username` = '$content'"); 
			}
			if($sousuo=='email'){				
				$arr=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `email` = '$content'"); 
			}
			if($sousuo=='mobile'){
				$arr=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `mobile` = '$content'");			
			}
		}	
...
HTML;
	...
	public function recharge(){
		...
		$num=20;
		if(empty($wheres)){
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户') AND `type`='1'"); 
			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户') AND `type`='1'"); 
		}else{
		...
		$page->config($total,$num,$pagenum,"0");	
		if(empty($wheres)){
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户') AND `type`='1' order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		}else{
