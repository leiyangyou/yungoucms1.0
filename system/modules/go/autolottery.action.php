...
class autolottery extends SystemAction {
	...
	public function init(){
		...
		$jinri_shoplist = $this->db->GetList("select * from `@#_shoplist` where `xsjx_time` > '$w_jinri_time' and `xsjx_time` < '$w_minri_time' limit 0,3");
		$minri_shoplist = $this->db->GetList("select * from `@#_shoplist` where `xsjx_time` > '$w_minri_time' and `xsjx_time` < '$w_hinri_time' limit 0,3");	
		
		//往期回顾
		$endshoplist = $this->db->GetList("select * from `@#_shoplist` where `xsjx_time` != '' and `q_end_time` != '' limit 0,8");
