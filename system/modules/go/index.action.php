...
class index extends base {
	...
	public function init(){		
		...
		//最新揭晓
		$shopqishu=$this->db->GetList("select id,sid,thumb,title,q_uid,q_user from `@#_shoplist` where `q_end_time` is not null ORDER BY `q_end_time` DESC LIMIT 4");
		//云购动态
	...
	public function glist(){
		...
		if(empty($fen1)){
			...
			$daohang=$this->db->GetOne("select * from `@#_category` where `cateid` = '$fen1' LIMIT 1");
		...
		if($fen1 and $fen2){			
			...
			$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `q_uid` is null and `cateid`='$fen1' $select_w",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{			
	...
	public function item(){
		...
		$itemlist = $this->db->GetList("select id,qishu from `@#_shoplist` where `sid`='$item[sid]' and `q_uid` is not null order by `qishu` DESC");
		
		//期数显示
		$loopqishu='';
		$loopqishu.='<li><a href="'.WEB_PATH.'/goods/'.$item['id'].'"><b class="period_Ongoing period_ArrowCur">'."第".$item['qishu']."期<i></i></b></a></li>";
		if($q_showtime=='Y')
		if(count($itemlist)>2)unset($itemlist[0]);
	...
	public function dataserver(){
		...
		$itemlist = $this->db->GetList("select id,sid,qishu,q_end_time from `@#_shoplist` where `sid`='$item[sid]' order by `qishu` DESC");
		...
		$user_shop_fmod = $item['q_user_code'];
		if($item['q_content']){
	...
	public function lottery(){	
		//最新揭晓
		$page=System::load_sys_class('page');		
		$total=$this->db->GetCount('select * from `@#_shoplist` where `q_uid` is not null');
		if(isset($_GET['p'])){
			$pagenum=$_GET['p'];
		}else{
			$pagenum=1;
		...
		$num=20;
		$page->config($total,$num,$pagenum,"0");
		$shopqishu=$this->db->GetPage("select * from `@#_shoplist` where `q_uid` is not null ORDER BY `q_end_time` DESC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));	
