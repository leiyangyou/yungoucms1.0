...
class setting extends admin {	
	...
	public function config(){
		...
		if(isset($_POST['dosubmit'])){		
			...
			$web_off_text=htmlspecialchars($_POST['web_off_text']);
	...
	public function webcfg(){
		if(isset($_POST['dosubmit'])){
			...
			$web_copyright=htmlspecialchars($_POST['web_copyright']);
...
HTML;
	...
	public function mobile(){
		...
		$cesi=$this->segment(4);
		if($cesi=='cesi'){
			$haoma=$this->segment(5);			
			$sendok=_sendmobile($haoma,$this->_cfg['web_name']."短信测试成功:".date("Y-m-d H:i:s",time()));
			if($sendok[0]!=1){
				echo "短信发送失败 ---  错误号:".$sendok[1];
			}else{
				echo "短信发送成功!";
			}
			exit;
...
HTML;
		...
		if($sendmobile->error==1){
			$text="<b style='color:#0c0'>短信功能正常</b>,短信还剩余 ".$sendmobile->v." 条";
		}else{
			$text="<b style='color:#ff0000'>短信测试失败</b>,失败原因:".$sendmobile->v;
		}
