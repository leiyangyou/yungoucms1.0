...
<div class="lr10 CMS_message" id="roll">
    ...
    <li>已购买授权用户，请加群交流运营经验：192428647【加群须备注授权域名】</li>
    <li>已购买授权用户，遇到任何技术问题，可到官网(<a target="_black" href="http://www.yungoucms.com">YunGouCMS</a>)>会员中心提交工单，我们会优先处理！</li>
</div>
<div class="bk10"></div>
...
<!------------>
	...
	<?php
	$tj_category=$this->db->GetList("SELECT * FROM `@#_category` WHERE `model` = '1' ");
	$tj_brand=$this->db->GetList("SELECT * FROM `@#_brand`");
	$tj_article=$this->db->GetList("SELECT * FROM `@#_article`");
	$tj_shoplist=$this->db->GetList("SELECT * FROM `@#_shoplist`");	
	$time=time();
	$tj_shoplist_xsjx=$this->db->GetList("SELECT * FROM `@#_shoplist` where `xsjx_time`>'$time'");
	$tj_member=$this->db->GetList("SELECT * FROM `@#_member`");
	
	$tm=time()-24*3600;
	$tj_member_new=$this->db->GetList("SELECT * FROM `@#_member` where `time`>'$tm' ");
	$tj_shoplist_new=$this->db->GetList("SELECT * FROM `@#_shoplist` where `time`>'$tm' ");
	$tj_member_account=$this->db->GetList("SELECT * FROM `@#_member_account` where `pay`='账户' and `type`=1 and `time`>'$tm'");
	$today_money=0;
    ...
    <div class="div-about width30">
        ...
        <div class="bk10"></div>
        <ul>
        	<li><i>程序版本:</i><?php echo $versions['version']; ?><font color="#f60">【<?php echo $text; ?>】</font></li>
			<li><i>更新时间:</i><?php echo $versions['release']; ?></li>
            <li><i>程序开发:</i>重庆韬龙网络科技有限公司</li>
            <li><i>版权所有:</i>重庆韬龙网络科技有限公司</li>    
            <li><i>官方微博:</i><a href="http://weibo.com/cqtaolong" target="_black" style="color:#0f0">关注官方微博</a></li>        
        </ul>
